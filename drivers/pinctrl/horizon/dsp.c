...
static const struct horizon_pinctrl_priv horizon_dsp_pinctrl_info = {
	.pins  = horizon_dsp_pins_desc,
	.npins = ARRAY_SIZE(horizon_dsp_pins_desc),
};

static inline int horizon_dsp_pinctrl_set_state(struct udevice *dev, struct udevice *config)
{
	return horizon_pinctrl_set_state(dev, config, &horizon_dsp_pinctrl_info);
}

static inline int horizon_dsp_pinctrl_probe(struct udevice *dev)
{
	return horizon_pinctrl_probe(dev, &horizon_dsp_pinctrl_info);
}

static const struct pinctrl_ops horizon_pinctrl_dsp_ops = {
	.set_state              = horizon_dsp_pinctrl_set_state,
};

static const struct udevice_id horizon_dsp_pinctrl_of_match[] = {
	{
		.compatible = "d-robotics,horizon-dsp-iomuxc",
		.data = (ulong)&horizon_dsp_pinctrl_info,
	},
	{ }
};

U_BOOT_DRIVER(horizon_dsp_pinctrl) = {
	.name           = "horizon_dsp_pinctrl",
	.id             = UCLASS_PINCTRL,
	.of_match       = of_match_ptr(horizon_dsp_pinctrl_of_match),
	.priv_auto      = sizeof(struct horizon_pinctrl_priv),
	.ops            = &horizon_pinctrl_dsp_ops,
